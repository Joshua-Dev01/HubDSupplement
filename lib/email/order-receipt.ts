...
export function customerReceiptHTML(order: OrderDetails) {
  ...
  <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; color: #1F2421;">
    <div style="background: #1F2421; padding: 24px; text-align: center;">
      <h1 style="color: #fff; font-size: 18px; letter-spacing: 2px; margin: 0; text-transform: uppercase;">HubDsupplement</h1>
    </div>
    ...
    <div style="padding: 32px 24px;">
      ...
      <table style="width: 100%; margin-top: 16px;">
        ...
        <tr>
          ...
          <td style="padding: 4px 0; font-size: 14px; text-align: right;">${formatNaira(order.tax)}</td>
        </tr>
        <tr>
      ...
      <div style="margin-top: 32px; padding: 16px; background: #F7F5F0; border-radius: 6px;">
        <p style="font-size: 12px; text-transform: uppercase; letter-spacing: 1px; color: #888; margin: 0 0 8px;">Delivery Address</p>
        <p style="font-size: 14px; margin: 0; color: #1F2421;">${order.full_name}</p>
        <p style="font-size: 14px; margin: 0; color: #1F2421;">${order.address}</p>
        <p style="font-size: 14px; margin: 0; color: #1F2421;">${order.city}, ${order.state}</p>
        <p style="font-size: 14px; margin: 0; color: #1F2421;">${order.phone}</p>
      </div>
...
export function adminNotificationHTML(order: OrderDetails) {
  ...
  <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; color: #1F2421;">
    ...
    <div style="padding: 24px;">
      ...
      <a href="${process.env.NEXT_PUBLIC_SITE_URL}/admin/orders" style="display: inline-block; margin-top: 16px; background: #1F2421; color: #fff; padding: 10px 20px; text-decoration: none; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">
        View in Admin Panel
      </a>
    </div>
  </div>
