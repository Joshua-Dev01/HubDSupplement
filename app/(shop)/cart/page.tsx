    ...
    <div className="bg-[#F7F5F0] min-h-screen pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-8">
          ...
          <p className="text-sm text-[#8A928E] mt-1">
            {items.length} item{items.length !== 1 ? 's' : ''} in your cart
          </p>
        </div>

        {items.length === 0 ? (
          <div className="bg-white rounded-2xl py-24 text-center flex flex-col items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-[#EFEDE6] flex items-center justify-center">
              <ShoppingBag size={22} className="text-[#8A928E]" />
            </div>
            <div>
              <p className="text-sm font-medium text-[#1F2421] mb-1">Your cart is empty</p>
              <p className="text-xs text-[#8A928E]">
                {loading ? 'Checking for saved items...' : 'Looks like you haven\'t added anything yet'}
              </p>
            </div>
            <Link
              href="/shop"
              className="mt-2 bg-[#5F7A5B] hover:bg-[#4F6A4B] text-white text-sm font-medium px-6 py-3 rounded-full transition-colors"
            >
              Start Shopping
            </Link>
          </div>
