...
function generatePaymentReference() {
  ...
  return `HDS_${timestamp}_${random}`
}

export default function CheckoutPage() {
  const { items, updateQuantity, removeItem, authUserEmail, authUserId, init } = useCartStore()
  ...
  const inputClass = "w-full bg-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#5F7A5B] transition-all"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div>
                ...
                <div className="rounded-2xl p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input value={form.fullName} onChange={(e) => updateForm('fullName', e.target.value)} placeholder="Your full name" className={inputClass} />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-widest text-[#8A928E] mb-1.5 block">Email</label>
                    <input type="email" value={form.email} onChange={(e) => updateForm('email', e.target.value)} placeholder="you@example.com" className={inputClass} />
                  </div>
                  <div>
                    <label className="text-xs uppercase tracking-widest text-[#8A928E] mb-1.5 block">Phone Number</label>
                    <input type="tel" value={form.phone} onChange={(e) => updateForm('phone', e.target.value)} placeholder="080X XXX XXXX" className={inputClass} />
                  </div>
                  <div className="md:col-span-2">
                    <label className="text-xs uppercase tracking-widest text-[#8A928E] mb-1.5 block">Delivery Address</label>
                    <input value={form.address} onChange={(e) => updateForm('address', e.target.value)} placeholder="Street address" className={inputClass} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select value={form.state} onChange={(e) => updateForm('state', e.target.value)} className={inputClass}>
            ...
            <div className="lg:col-span-4 bg-white rounded-2xl p-6">
              ...
              <div className="space-y-3 text-sm border-b border-black/10 pb-6">
                <div className="flex justify-between text-[#8A928E]"><span>Subtotal</span><span className="text-[#1F2421] font-medium">{formatNaira(subtotal)}</span></div>
                <div className="flex justify-between text-[#8A928E]"><span>Shipping</span><span className="text-[#1F2421] font-medium">Free</span></div>
                <div className="flex justify-between text-[#8A928E]"><span>Estimated Tax</span><span className="text-[#1F2421] font-medium">{formatNaira(tax)}</span></div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <input value={promoCode} onChange={(e) => setPromoCode(e.target.value)} placeholder="Enter code" className="flex-1 border border-black/10 rounded-full px-4 py-2.5 text-sm outline-none focus:border-[#5F7A5B] transition-colors" />
                  <button onClick={handleApplyPromo} className="bg-[#5F7A5B] cursor-pointer hover:bg-[#4F6A4B] text-white text-sm font-medium px-5 py-2.5 rounded-full transition-colors">Apply</button>
                </div>
              </div>
